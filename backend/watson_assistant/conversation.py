from .watson import WatsonAPI, WatsonArabicAPI
...
class Conversation_Arabic():
    def __init__(self):
        self.wapi = WatsonArabicAPI()
        self.gapi = GoogleAPI()
    ...
    def find_route(self, params, context, result):
        response = self.gapi.find_route(context)
        response['time'] = self.wapi.translate_to_arabic(response['time'])
        context[result] = response
        return context
    ...
    def message(self, message, context):
        ...
        if('output' in wapi_response and 'context' in wapi_response):
            ...
            return {
                'text': 'هناك شيء خاطئ. يرجى إعادة تحميل الصفحة وحاول مرة أخرى.',
                'context': {}
            }
    def start(self):
        start_param = 'hjukwkJIgAlPEUHd'
